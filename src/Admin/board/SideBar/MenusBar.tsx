    ...
    return (
        <>
            {(parentActive == 'Menus-ON' || parentActive == 'Menus-Mini-ON') && (
                <div className={`fixed flex-shrink-0 w-64 bg-background-light dark:bg-background-dark border-indigo-100 border-r-2 shadow-lg  rounded-tr-3xl rounded-br-3xl lg:static py-5 px-1 ${parentActive === 'Menus-Mini-ON' ? 'mt-[64px] h-[calc(100vh-64px)]': ''} `}>
                {/* <div className="fixed flex-shrink-0 w-64 bg-background-light dark:bg-background-dark border-indigo-100 border-r-2 shadow-lg sm:left-16 rounded-tr-3xl rounded-br-3xl lg:static py-5 px-1"> */}
                                ...
                                >
                                    ...
                                    {menu.childMenu.length > 0 && (
                                        <span className="ml-auto">
                                            {expandedMenu === menu.unikName ? "🠉" : "🠋"}
                                        </span>
                                    )}
                                </h6>
                                {menu.childMenu.length > 0 && expandedMenu === menu.unikName && (
                                    <ul className="pl-5">
                                        {menu.childMenu.map((child) => (
                                            <li key={child.unikName}>
                                                <h6
                                                    className={`flex items-center p-2 ${child.unikName === child.unikName ? "text-primary-light" : "text-gray-900"} rounded-lg dark:text-white hover:text-primary-light dark:hover:bg-gray-700 group`}
                                                >
